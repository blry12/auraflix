	...
	def _builder():
		for count, item in enumerate(cloud_dict, 1):
			try:
				...
				linked_folder = test_assigned_content('afm_AD_%s' % folder_id, assigned_content)
				...
				info_tag.setPlot(' ')
				listitem.setProperty('afm.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': clean_folder_name, 'iconImage': default_ad_icon,
									'service': 'AD'}))
				yield (url, listitem, True)
			except: pass
	try: cloud_dict = [i for i in AllDebrid.user_cloud()['magnets'] if i['statusCode'] == 4]
	except: cloud_dict = []
	if cloud_dict: assigned_content = gather_assigned_content("'afm_AD_%'")
	...
	def _builder():
		for count, item in enumerate(links, 1):
			try:
				...
				info_tag = listitem.getVideoInfoTag()
				info_tag.setPlot(' ')
				listitem.setProperty('afm.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': default_ad_icon,
