...
base_str, window_prop = '[B]%s:[/B]  [I]%s[/I]', 'afm.%s_discover_params'
...
class Discover:
		...
		def _builder():
			for count, item in enumerate(data):
				try:
					...
					listitem.setProperty('afm.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': default_icon}))
					yield (url, listitem, True)
				except: pass
		handle = int(sys.argv[1])
		media_type = media_type if media_type else self.media_type
		string = 'afm_discover_%s_%%' % media_type
	...
	def help(self):
		text_file = translate_path('special://home/addons/plugin.video.afm/resources/text/tips/135. Discover.txt')
...
def set_history(media_type, name, query):
	string = 'afm_discover_%s_%s' % (media_type, query)
