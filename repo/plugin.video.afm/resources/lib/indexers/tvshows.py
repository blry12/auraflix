...
afm_str, trakt_str, watched_str, unwatched_str, exit_str, nextpage_str, browse_str, jump2_str = ls(32036), ls(32037), ls(32642), ls(32643), ls(32650), ls(32799), ls(33137), ls(32964)
...
class TVShows:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external:
				...
				if not any([x in folderpath for x in internal_nav_check]): set_property('afm.exit_params', folderpath)
	...
	def build_tvshow_content(self, _position, _id):
		try:
			...
			set_properties({'afm.extras_params': extras_params, 'afm.options_params': options_params})
	...
	def worker(self):
		...
		self.is_folder, self.watched_title = False if self.open_extras else True, trakt_str if self.watched_indicators == 1 else afm_str
