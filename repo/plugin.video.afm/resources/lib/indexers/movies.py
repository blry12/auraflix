...
afm_str, trakt_str, watched_str, unwatched_str, extras_str, options_str = ls(32036), ls(32037), ls(32642), ls(32643), ls(32645), ls(32646)
...
class Movies:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external: set_property('afm.exit_params', folder_path())
	...
	def build_movie_content(self, _position, _id):
		try:
			...
			set_properties({'afm.extras_params': extras_params, 'afm.clearprog_params': clearprog_params, 'afm.options_params': options_params,
							'afm.unwatched_params': watched_status_params, 'afm.watched_params': watched_status_params})
	...
	def worker(self):
		...
		self.open_extras, self.watched_title = extras_open_action('movie'), trakt_str if self.watched_indicators == 1 else afm_str
	...
	def build_movie_sets_content(self, _position, _id):
		try:
			...
			if self.is_external:
				cm_append((refr_widg_str, run_plugin % build_url({'mode': 'kodi_refresh'})))
				set_property('afm.external', 'true')
			info_tag = listitem.getVideoInfoTag()
			info_tag.setMediaType('movie')
			...
			set_property('afm.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': title, 'iconImage': poster}))
