	...
	def _builder():
		for count, item in enumerate(cloud_files, 1):
			try:
				...
				linked_folder = test_assigned_content('afm_RD_%s' % folder_id, assigned_content)
				...
				info_tag.setPlot(' ')
				listitem.setProperty('afm.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': clean_folder_name, 'iconImage': default_rd_icon,
									'service': 'RD'}))
				yield (url, listitem, True)
			except: pass
	try: cloud_files = [i for i in RealDebrid.user_cloud() if i['status'] == 'downloaded']
	except: cloud_files = []
	if cloud_files: assigned_content = gather_assigned_content("'afm_RD_%'")
	...
	def _builder():
		for count, item in enumerate(downloads, 1):
			try:
				...
				info_tag = listitem.getVideoInfoTag()
				info_tag.setPlot(' ')
				listitem.setProperty('afm.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': default_rd_icon,
...
def resolve_rd(params):
	...
	afmPlayer().run(resolved_link, 'video')
