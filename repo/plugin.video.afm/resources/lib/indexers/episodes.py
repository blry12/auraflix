...
string, afm_str, trakt_str, watched_str, unwatched_str, season_str, episodes_str =  str, ls(32036), ls(32037), ls(32642), ls(32643), ls(32537), ls(32506)
...
def build_episode_list(params):
		...
		for item in episodes_data:
			try:
				...
				if is_external:
					...
					set_properties({'afm.external': 'true'})
				...
				set_properties({'afm.extras_params': extras_params, 'afm.options_params': options_params, 'afm.unwatched_params': unwatched_params,
								'afm.watched_params': watched_params, 'afm.clearprog_params': clearprog_params, 'episode_type': episode_type})
	...
	watched_title = trakt_str if watched_indicators == 1 else afm_str
...
def build_single_episode(list_type, params={}):
	...
	def _process(_position, ep_data):
		try:
			...
			if list_type_starts_with('next_'):
				...
				if nextep_include_airdate: airdate = '[COLOR %s][%s][/COLOR] ' % (afm_highlight, display_premiered)
				else: airdate = ''
				...
				if unaired:
					...
					display = '[COLOR %s]%s][/COLOR]%s' % (afm_highlight, displays[0], displays[1])
			else:
				color_tags = ('[COLOR %s]' % afm_highlight, '[/COLOR]') if unaired else ('', '')
	...
	handle, is_external, category_name = int(sys.argv[1]), external(), episodes_str
	item_list, unwatched = [], []
	resinsert, afm_highlight = '', get_property('afm.main_highlight')
	...
	watched_info, bookmarks, watched_title = get_watched_info(watched_indicators), get_bookmarks(watched_indicators, 'episode'), trakt_str if watched_indicators == 1 else afm_str
