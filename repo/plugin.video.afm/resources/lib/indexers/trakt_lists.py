	...
	def _builder():
		for item in lists:
			try:
				...
				listitem.setProperty('afm.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': editor_display, 'iconImage': 'trakt'}))
...
def build_trakt_list(params):
	...
	try:
		...
		if page_no == 1 and not is_external: set_property('afm.exit_params', folder_path())
