...
class Navigator:
	...
	def accounts_manager(self):
		trakt_active = get_setting('afm.trakt.user') != ''
	...
	def changelog_utils(self):
		afm_clogpath = tp(log_path % 'plugin.video.afm/resources/text')
		mh_str = '[B]%s[/B]: %s  [I](v.%s)[/I]' % (changelog_str.upper(), afm_str, addon().getAddonInfo('version'))
		self.add({'mode': 'show_text', 'heading': mh_str, 'file': afm_clogpath, 'font_size': 'large', 'isFolder': 'false'}, mh_str, 'lists', False)
	...
	def folder_navigator(self):
		...
		def _process():
			for info in results:
				try:
					...
					linked_folder = test_assigned_content('afm_FOLDERS_%s' % link_id, assigned_content)
		...
		assigned_content = gather_assigned_content("'afm_FOLDERS_%s" % setting_id + "_%'")
		...
		def _builder():
			for item in folder_info:
				for media_type in ('movie', 'tvshow'):
					...
					display_name = get_setting('afm.%s.display_name' % setting_id)
					if action == 'browse':
	...
	def exit_media_menu(self):
		params = get_property('afm.exit_params')
		if params: return container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.afm/resources/text/tips'
	...
	def add(self, url_params, list_name, iconImage='folder', contextmenu_edit=True, original_image=False):
		...
		if contextmenu_edit:
			...
			listitem.addContextMenuItems(cm)
			listitem.setProperty('afm.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': list_name, 'iconImage': iconImage}))
			if self.is_home: listitem.setProperty('afm.external', 'true')
