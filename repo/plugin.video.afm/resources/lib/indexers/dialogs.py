...
single_ep_list = ('episode.progress', 'episode.recently_watched', 'episode.next_trakt', 'episode.next_afm', 'episode.trakt_recently_aired', 'episode.trakt_calendar')
...
def link_folders_choice(params):
	...
	string = 'afm_%s_%s' % (service, folder_id)
...
def navigate_to_page_choice(params):
		...
		for i in start_list:
			try:
				if i == 1 and widget_content: line1 = from_widget_str
					...
					if i == current_page: line1 = '[COLOR %s][B]%s   |   %s[/B][/COLOR]' % (afm_highlight, line1, current_page_str)
				else:
	...
	try:
		...
		afm_highlight = get_property(highlight_prop)
...
def trakt_manager_choice(params):
	if not get_setting('afm.trakt.user', ''): return notification(32760, 3500)
...
def set_quality_choice(params):
	...
	fl = ['SD', '720p', '1080p', '4K']
	try: preselect = [fl.index(i) for i in get_setting('afm.%s' % quality_setting).split(', ')]
	except: preselect = []
	...
	if not orig_button_dict:
		for _type in ('movie', 'tvshow'):
			...
			for item in range(10, 18):
				...
				button_action = get_setting('afm.%s' % setting_id)
...
def set_language_filter_choice(params):
	...
	try: preselect = [fl.index(i) for i in get_setting('afm.%s' % filter_setting).split(', ')]
	except: preselect = []
...
def easynews_use_custom_farm_choice(params={}):
	...
	new_setting = 'True' if get_setting('afm.easynews.use_custom_farm', 'False') == 'False' else 'False'
...
def easynews_server_choice(params={}):
	...
	kwargs = {'items': json.dumps(list_items), 'heading': easy_serv_str, 'narrow_window': 'true'}
	port_choice = select_dialog(ports, **kwargs) or get_setting('afm.easynews.port', '443')
...
def enable_scrapers_choice(params={}):
	icon = params.get('icon', None) or get_icon('afm')
...
def scraper_quality_color_choice(params):
	...
	current_setting = get_setting('afm.%s' % setting)
...
def highlight_color_choice(params={}):
	current_setting = get_setting('afm.main_highlight')
	chosen_color = color_choice({'current_setting': current_setting})
	if chosen_color:
		manage_settings_reset()
...
def options_menu_choice(params, meta=None):
	...
	if menu_type in ('movie', 'tvshow'):
		if get_setting('afm.trakt.user', ''): listing_append((ls(32198), '', 'trakt_manager'))
