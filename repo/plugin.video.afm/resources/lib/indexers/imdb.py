	...
	def _builder():
		for item in user_lists:
			try:
				...
				info_tag.setPlot(' ')
				listitem.setProperty('afm.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': title, 'iconImage': 'imdb'}))
	...
	def _builder():
		for count, keyword in enumerate(results, 1):
			try:
				...
				listitem.addContextMenuItems(cm)
				listitem.setProperty('afm.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': 'imdb'}))
