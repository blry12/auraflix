...
user_agent = 'afm_for_kodi'
timeout = 20.0
icon = get_icon('alldebrid')

class AllDebridAPI:
	def __init__(self):
		self.token = get_setting('afm.ad.token')
	...
	def clear_cache(self, clear_hashes=True):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('afm_ad_user_cloud',))
				clear_property('afm_ad_user_cloud')
				dbcon.commit()
				user_cloud_success = True
			except: user_cloud_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('afm_ad_valid_hosts',))
				clear_property('afm_ad_valid_hosts')
				dbcon.commit()
				dbcon.close()
