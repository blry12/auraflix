...
class FurkAPI:
	...
	def get_api(self):
		api_key = get_setting('afm.furk_api_key', '')
		if not api_key:
			try:
				user_name, user_pass = get_setting('afm.furk_login'), get_setting('afm.furk_password')
	...
	def direct_search(self, query):
		try:
			...
			string = 'afm_FURK_SEARCH_DIRECT_%s' % url
	...
	def t_files(self, file_id):
		try:
			...
			string = 'afm_%s_%s' % ('FURK_T_FILE', file_id)
...
def clear_media_results_database():
	...
	dbcur.execute("SELECT id FROM maincache WHERE id LIKE 'afm_FURK_SEARCH_%'")
	try:
		furk_results = [str(i[0]) for i in dbcur.fetchall()]
		if not furk_results: results.append('success')
		dbcur.execute("DELETE FROM maincache WHERE id LIKE 'afm_FURK_SEARCH_%'")
		for i in furk_results: clear_property(i)
		results.append('success')
	except: results.append('failed')
	dbcur.execute("SELECT id FROM maincache WHERE id LIKE 'afm_FURK_SEARCH_DIRECT_%'")
	try:
		furk_results = [str(i[0]) for i in dbcur.fetchall()]
		if not furk_results: results.append('success')
		dbcur.execute("DELETE FROM maincache WHERE id LIKE 'afm_FURK_SEARCH_DIRECT_%'")
		for i in furk_results: clear_property(i)
		results.append('success')
	except: results.append('failed')
