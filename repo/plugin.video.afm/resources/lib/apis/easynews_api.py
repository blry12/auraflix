...
def import_easynews():
	...
	# if get_setting('afm.easynews.api_version') == '0': return EasyNewsAPI()
...
class EasyNewsAPI:
	def __init__(self):
		...
		self.username = get_setting('afm.easynews_user')
		self.password = get_setting('afm.easynews_password')
	...
	def search(self, query, expiration=48):
		...
		string = 'afm_EASYNEWS_SEARCH_' + urlencode(self.params)
		...
		def _process():
			for item in files:
				try:
					...
					from modules.kodi_utils import logger
					logger('afm easynews API Exception', str(e))
	...
	def get_farm_and_port(self, files):
		...
		if get_setting('afm.easynews.use_custom_farm', 'False') == 'True': dl_farm, dl_port = get_setting('afm.easynews.farm', dl_farm), get_setting('afm.easynews.port', dl_port)
...
def clear_media_results_database():
	...
	dbcur.execute("SELECT id FROM maincache WHERE id LIKE 'afm_EASYNEWS_SEARCH_%'")
	easynews_results = [str(i[0]) for i in dbcur.fetchall()]
	if not easynews_results: return 'success'
	try:
		dbcur.execute("DELETE FROM maincache WHERE id LIKE 'afm_EASYNEWS_SEARCH_%'")
