...
CLIENT_ID, CLIENT_SECRET = '3a69ec1b3b579246dbd6a2d53f49d6b5f227d82d75fe048c1c6ea684a3668ebc', 'e98253e2f69c2a72a401af82059c866bfa0853bbc2d27a01869537981295d388'
...
def call_trakt(path, params={}, data=None, is_delete=False, with_auth=True, method=None, pagination=False, page_no=1):
	def send_query():
		...
		if with_auth:
			try:
				try: expires_at = float(get_setting('afm.trakt.expires'))
				except: expires_at = 0.0
				if time.time() > expires_at: trakt_refresh_token()
			except: pass
			token = get_setting('afm.trakt.token')
...
def trakt_refresh_token():
	data = {        
		...
		'grant_type': 'refresh_token', 'refresh_token': get_setting('afm.trakt.refresh')}
...
def trakt_revoke_authentication(dummy=''):
	data = {'token': get_setting('afm.trakt.token'), 'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET}
...
def trakt_calendar_days(recently_aired, current_date):
	if recently_aired: start, finish = (current_date - timedelta(days=14)).strftime('%Y-%m-%d'), '14'
		...
		previous_days = int(get_setting('afm.trakt.calendar_previous_days', '3'))
		future_days = int(get_setting('afm.trakt.calendar_future_days', '7'))
...
def trakt_sync_activities(force_update=False):
	...
	if not get_setting('afm.trakt.user', '') and not force_update: return 'no account'
