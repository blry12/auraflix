...
class RealDebridAPI:
	def __init__(self):
		self.client_ID = get_setting('afm.rd.client_id')
		if self.client_ID == '': self.client_ID = 'X245A4XAIBGVM'
		self.token = get_setting('afm.rd.token')
		self.secret = get_setting('afm.rd.secret')
		self.refresh = get_setting('afm.rd.refresh')
		self.device_code = ''
	...
	def user_cloud(self):
		string = 'afm_rd_user_cloud'
		url = 'torrents?limit=500'
		return cache_object(self._get, string, url, False, 0.5)

	def downloads(self):
		string = 'afm_rd_downloads'
		url = 'downloads?limit=500'
		return cache_object(self._get, string, url, False, 0.5)

	def user_cloud_info(self, file_id):
		string = 'afm_rd_user_cloud_info_%s' % file_id
	...
	def clear_cache(self, clear_hashes=False):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT data FROM maincache WHERE id=?""", ('afm_rd_user_cloud',))
				try: 
				...
				if not user_cloud_success:
					dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('afm_rd_user_cloud',))
					clear_property("afm_rd_user_cloud")
					for i in user_cloud_info_caches:
						dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('afm_rd_user_cloud_info_%s' % i,))
						clear_property("afm_rd_user_cloud_info_%s" % i)
					dbcon.commit()
					user_cloud_success = True
			except: user_cloud_success = False
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('afm_rd_downloads',))
				clear_property("afm_rd_downloads")
				dbcon.commit()
				download_links_success = True
			except: download_links_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('afm_rd_valid_hosts',))
				clear_property('afm_rd_valid_hosts')
				dbcon.commit()
				dbcon.close()
