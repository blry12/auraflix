...
class PremiumizeAPI:
	def __init__(self):
		...
		self.user_agent = 'afm for Kodi'
		self.token = get_setting('afm.pm.token')
	...
	def user_cloud(self, folder_id=None):
		if folder_id:
			string = 'afm_pm_user_cloud_%s' % folder_id
			url = 'folder/list?id=%s' % folder_id
		else:
			string = 'afm_pm_user_cloud_root'
			url = 'folder/list'
		return cache_object(self._get, string, url, False, 0.5)

	def user_cloud_all(self):
		string = 'afm_pm_user_cloud_all_files'
		url = 'item/listall'
		return cache_object(self._get, string, url, False, 0.5)
	...
	def get_item_details(self, item_id):
		string = 'afm_pm_item_details_%s' % item_id
		url = 'item/details'
		data = {'id': item_id}
		args = [url, data]
	...
	def clear_cache(self, clear_hashes=True):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT id FROM maincache WHERE id LIKE ?""", ('afm_pm_user_cloud%',))
				try:
			...
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('afm_pm_transfers_list',))
				clear_property("afm_pm_transfers_list")
				dbcon.commit()
				download_links_success = True
			except: download_links_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('afm_pm_valid_hosts',))
				clear_property('afm_pm_valid_hosts')
				dbcon.commit()
				dbcon.close()
