...
class source:
	def __init__(self, scrape_provider, scraper_name, folder_path):
		...
		self.assignment_test_string = 'afm_FOLDERS_%s_%s' % (self.scrape_provider, '%s')
	...
	def results(self, info):
		try:
			...
			from modules.kodi_utils import logger
			logger('afm folders scraper Exception', str(e))
		internal_results(self.scraper_name, self.sources)
		return self.sources

	def _gather_assigned_content(self):
		pre_assigned_content = gather_assigned_content("'afm_FOLDERS_%s" % self.scrape_provider + "_%'")
	...
	def _scrape_directory(self, folder_info, first_run=False):
		...
		string = 'afm_FOLDERSCRAPER_%s_%s' % (self.scrape_provider, folder_name)
