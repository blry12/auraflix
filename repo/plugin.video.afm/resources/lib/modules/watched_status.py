...
afm_str, trakt_str, watched_str, unwatched_str, season_str = ls(32036), ls(32037), ls(32642), ls(32643), ls(32537)
indicators_dict = {0: WATCHED_DB, 1: TRAKT_DB}
finished_show_check = ('Ended', 'Canceled')
progress_db_string = 'afm_hidden_progress_items'
...
def clear_local_bookmarks():
	try:
		...
		dbcur = set_PRAGMAS(dbcon)
		file_ids = dbcur.execute("SELECT idFile FROM files WHERE strFilename LIKE 'plugin.video.afm%'").fetchall()
