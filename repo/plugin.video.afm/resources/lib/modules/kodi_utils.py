...
addon_object = xbmcaddon.Addon('plugin.video.afm')
...
addon_icon = path_join(addon_path, 'resources', 'media', 'afm_icon.png')
addon_fanart = addon_object.getSetting('addon_fanart') or path_join(addon_path, 'resources', 'media', 'afm_fanart.png')
...
def build_url(url_params):
	return 'plugin://plugin.video.afm/?%s' % urlencode(url_params)
...
def end_directory(handle, cacheToDisc=None):
	if cacheToDisc == None: cacheToDisc = get_setting('afm.kodi_menu_cache') == 'true'
...
def addon(addon_id='plugin.video.afm'):
...
def run_addon(addon='plugin.video.afm', block=False):
	return execute_builtin('RunAddon(%s)' % addon, block)

def external():
	return 'afm' not in get_infolabel('Container.PluginName')
...
def disable_enable_addon(addon_name='plugin.video.afm'):
	try:
...
def set_view(view_type):
	...
	set_property('afm.%s' % view_type, view_id)
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_setting('afm.playback.volumecheck_enabled', 'false') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_setting('afm.playback.volumecheck_percent', '50')), 100)
...
def toggle_language_invoker():
	...
	current_addon_setting = get_setting('afm.reuse_language_invoker', 'true')
...
def upload_logfile(params):
	...
	try:
		...
		UserAgent = 'afm %s' % addon_object.getAddonInfo('version')
...
def open_settings(query, addon='plugin.video.afm'):
	hide_busy_dialog()
	if query:
		try:
...
def get_setting(setting_id, fallback=''):
	return get_property(setting_id) or addon_object.getSetting(setting_id.replace('afm.', '')) or fallback
...
def make_settings_props():
	...
	try:
		...
		for item in all_settings(): set_property('afm.%s' % item[0], item[1])
