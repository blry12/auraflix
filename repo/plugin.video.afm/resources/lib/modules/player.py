	...
	def media_watched_marker(self, force_watched=False):
		...
		try:
			if self.current_point >= self.set_watched or force_watched:
				...
				clear_property('afm.random_episode_history')
...
class Subtitles(xbmc_player):
	def __init__(self):
		...
		self.auto_enable = get_setting('afm.subtitles.auto_enable')
		self.subs_action = get_setting('afm.subtitles.subs_action')
		self.language = get_setting('afm.subtitles.language_primary')
	...
	def get(self, query, imdb_id, season, episode, secondary_search=False):
		...
		sub_filename = 'afmSubs_%s_%s_%s' % (imdb_id, season, episode) if season else 'afmSubs_%s' % imdb_id
		...
		secondary_language = get_setting('afm.subtitles.language_secondary')
