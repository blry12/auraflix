...
trakt_sync_interval, trakt_sync_refresh_widgets, auto_start_afm = settings.trakt_sync_interval, settings.trakt_sync_refresh_widgets, settings.auto_start_afm
...
afm_str, window_top_str, listitem_property_str = ls(32036), 'Window.IsTopMost(%s)', 'ListItem.Property(%s)'
...
class InitializeDatabases:
	def run(self):
		logger(afm_str, 'InitializeDatabases Service Starting')
		check_databases()
		return logger(afm_str, 'InitializeDatabases Service Finished')

class CheckSettings:
	def run(self):
		logger(afm_str, 'CheckSettingsFile Service Starting')
		if not path_exists(userdata_path): make_directories(userdata_path)
		addon_object.setSetting('dummy_setting', 'foo')
		return logger(afm_str, 'CheckSettingsFile Service Finished')

class DatabaseMaintenance:
	def run(self):
		logger(afm_str, 'Database Maintenance Service Starting')
		time = datetime.datetime.now()
		current_time = self._get_timestamp(time)
		due_clean = int(get_setting('afm.database.maintenance.due', '0'))
		if due_clean == 0:
			next_clean = str(int(self._get_timestamp(time + datetime.timedelta(days=3))))
			set_setting('database.maintenance.due', next_clean)
			return logger(afm_str, 'Database Maintenance Service First Run - Skipping')
		if current_time >= due_clean:
			clean_databases(current_time, database_check=False, silent=True)
			next_clean = str(int(self._get_timestamp(time + datetime.timedelta(days=3))))
			set_setting('database.maintenance.due', next_clean)
			return logger(afm_str, 'Database Maintenance Service Finished')
		else: return logger(afm_str, 'Database Maintenance Service Finished - Not Run')
...
class FirstRunActions:
	def run(self):
		logger(afm_str, 'FirstRunActions Service Starting')
		if kodi_version() < 20:
			ok_dialog(afm_str, 'Kodi 20 or above required[CR]Please update Kodi or uninstall afm')
			return logger(afm_str, 'FirstRunActions Service Finished - Running on Incompatible Kodi Version')
		...
		if addon_version != settings_version:
			logger(afm_str, 'FirstRunActions Running Update Actions....')
			addon_object.setSetting('version_number', addon_version)
			self.update_action(addon_version)
		if get_setting('first_use', 'false') == 'true':
			lines = ('By default, afm has [B]Local Folders[/B], [B]Debrid Cloud[/B], [B]Furk[/B] and [B]Easynews[/B] scrapers.',
			'These scrapers are designed for use on media you have purchased and organized through these services.',
			'afm is [B]unable[/B] to search online for sources outside of these services.',
			'If you wish to use an external scraper package within afm, you must select a module at[CR][B]afm Settings->Providers->Choose External Scrapers Module.[/B]',
			'The use of external scrapers is outside of the intended use of afm, and will not be supported by afm.')
		...
		return logger(afm_str, 'FirstRunActions Service Finished')
...
class ReuseLanguageInvokerCheck:
	def run(self):
		logger(afm_str, 'ReuseLanguageInvokerCheck Service Starting')
		addon_xml = translate_path('special://home/addons/plugin.video.afm/addon.xml')
		current_addon_setting = get_setting('afm.reuse_language_invoker', None)
		if current_addon_setting is None: return logger(afm_str, 'ReuseLanguageInvokerCheck Service Error. No current setting detected. Finished')
		...
		if invoker_instance.data != current_addon_setting:
			...
			if not get_setting('afm.auto_invoker_fix') == 'true' and not confirm_dialog(text='%s[CR]%s' % (ls(33021), ls(33020))):
				return logger(afm_str, 'ReuseLanguageInvokerCheck Service Finished')
...
class TraktMonitor:
	def run(self):
		logger(afm_str, 'TraktMonitor Service Starting')
		...
		while not monitor.abortRequested():
			try:
				...
				if status in ('success', 'no account'):
					logger(afm_str, trakt_service_string % ('Success', success_line_dict[status]))
					if trakt_sync_refresh_widgets():
						kodi_refresh()
						logger(afm_str, trakt_service_string % ('Widgets Refresh', 'Setting Activated. Widget Refresh Performed'))
					else: logger(afm_str, trakt_service_string % ('Widgets Refresh', 'Setting Disabled. Skipping Widget Refresh'))
				elif status == 'failed': logger(afm_str, trakt_service_string % ('Failed. Error from Trakt', next_update_string))
				else: logger(afm_str, trakt_service_string % ('Success. No Changes Needed', next_update_string))# 'not needed'
			except Exception as e: logger(afm_str, trakt_service_string % ('Failed', 'The following Error Occured: %s' % str(e)))
			wait_for_abort(wait_time)
		try: del monitor
		except: pass
		try: del player
		except: pass
		return logger(afm_str, 'TraktMonitor Service Finished')

class CustomActions:
	def run(self):
		logger(afm_str, 'CustomActions Service Starting')
		monitor, player = xbmc_monitor(), xbmc_player()
		...
		while not abort_requested():
			...
			while not any([context_visible, info_visible]) and not abort_requested():
				...
				if not external() or get_infolabel(listitem_property_str % 'afm.external') == 'true':
					run_custom = True
					custom_context_params = get_infolabel(listitem_property_str % 'afm.options_params')
					custom_main_context_params = get_infolabel(listitem_property_str % 'afm.context_main_menu_params')
					custom_info_params = get_infolabel(listitem_property_str % 'afm.extras_params')
		...
		return logger(afm_str, 'CustomActions Service Finished')
...
class CustomFonts:
	def run(self):
		logger(afm_str, 'CustomFonts Service Starting')
		monitor, player = xbmc_monitor(), xbmc_player()
		wait_for_abort, is_playing = monitor.waitForAbort, player.isPlayingVideo
		...
		return logger(afm_str, 'CustomFonts Service Finished')

class ClearSubs:
	def run(self):
		logger(afm_str, 'Clear Subtitles Service Starting')
		sub_formats = ('.srt', '.ssa', '.smi', '.sub', '.idx', '.nfo')
		subtitle_path = 'special://temp/%s'
		files = list_dirs(translate_path('special://temp/'))[1]
		for i in files:
			if i.startswith('afmSubs_') or i.endswith(sub_formats): delete_file(translate_path(subtitle_path % i))
		return logger(afm_str, 'Clear Subtitles Service Finished')

class AutoRun:
	def run(self):
		logger(afm_str, 'AutoRun Service Starting')
		if auto_start_afm(): run_addon()
		return logger(afm_str, 'AutoRun Service Finished')

class PremiumExpiryCheck:
	def run(self):
		if not check_premium(): return
		logger(afm_str, 'Premium Expiry Check Service Starting')
		self.message, self.days_remaining = [], []
		try:
			self.run_check()
			if self.make_message(): ok_dialog('afm - Premium Accounts Expiring...', '\n'.join(self.message))
			self.expired_premium = [i for i in self.days_remaining if i[1] == 0]
			if self.expired_premium and confirm_dialog(heading='afm - Premium Accounts Expired', text='Disable Expired Accounts?', ok_label=32839, cancel_label=32840): self.revoke()
		except: pass
		logger(afm_str, 'Premium Expiry Check Service Finished')
...
class UpdateCheck:
	def run(self):
		logger(afm_str, 'UpdateCheck Service Starting')
		monitor, player = xbmc_monitor(), xbmc_player()
		wait_for_abort, is_playing = monitor.waitForAbort, player.isPlayingVideo
		while not monitor.abortRequested():
		...
		return logger(afm_str, 'UpdateCheck Service Finished')
	...
	def run(self, sender, method, data):
		if sender == 'xbmc':
			if method in ('GUI.OnScreensaverActivated', 'System.OnSleep'):
				...
				logger('OnNotificationActions', 'PAUSING afm Services Due to Device Sleep')
			elif method in ('GUI.OnScreensaverDeactivated', 'System.OnWake'):
				clear_property(pause_services_prop)
				logger('OnNotificationActions', 'UNPAUSING afm Services Due to Device Awake')
