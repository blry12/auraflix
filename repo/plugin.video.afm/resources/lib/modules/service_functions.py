...
afm_str, window_top_str, listitem_property_str = ls(32036).upper(), 'Window.IsTopMost(%s)', 'ListItem.Property(%s)'
...
class FirstRunActions:
	def run(self):
		logger(afm_str, 'CheckUpdateActions Service Starting')
		...
		if addon_version != settings_version:
			logger(afm_str, 'CheckUpdateActions Running Update Actions....')
		...
		return logger(afm_str, 'CheckUpdateActions Service Finished')
...
class ReuseLanguageInvokerCheck:
	def run(self):
		...
		if invoker_instance.data != current_addon_setting:
			...
			if not get_setting('afm.auto_invoker_fix') == 'true' and not confirm_dialog(text='%s\n%s' % (ls(33021), ls(33020))):
