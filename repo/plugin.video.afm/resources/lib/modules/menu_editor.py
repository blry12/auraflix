...
default_path = 'plugin://plugin.video.afm'
...
class MenuEditor:
	def __init__(self, params):
		...
		else: self.menu_item = dict(parse_qsl(get_infolabel('ListItem.FileNameAndPath').replace('plugin://plugin.video.afm/?','')))
	...
	def edit_menu(self):
		...
		if get_setting('afm.trakt.user', ''): append((trak_add_str % list_name, self.add_trakt))
	...
	def shortcut_folder_browse_for_content(self):
		...
		self.menu_item = dict(parse_qsl(browsed_result['file'].replace('plugin://plugin.video.afm/?','')))
