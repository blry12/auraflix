...
class Sources():
	...
	def playback_prep(self, params=None):
		...
		self.folders_ignore_filters = get_setting('afm.results.folders_ignore_filters', 'false') == 'true'
		...
		self.hybrid_allowed, self.filter_size_method = self.filter_hdr in (0, 2), int(get_setting('afm.results.filter_size_method', '0'))
		self.include_unknown_size = get_setting('afm.results.include.unknown.size', 'false') == 'true'
		self.include_3D_results = get_setting('afm.include_3d_results', 'true') == 'true'
		self._update_meta()
		self._search_info()
	...
	def filter_results(self, results):
		...
		if self.filter_size_method:
			min_size = string_to_float(get_setting('afm.results.size.min_manual', '0'), '0') / 1000
			...
			if self.filter_size_method == 1:
				...
				max_size = ((0.125 * (0.90 * string_to_float(get_setting('afm.results.size.auto', '20'), '20'))) * duration)/1000
			elif self.filter_size_method == 2:
				max_size = string_to_float(get_setting('afm.results.size.manual', '10000'), '10000') / 1000
	...
	def get_folderscraper_info(self):
		folder_info = [(get_setting('afm.%s.display_name' % i), i, source_folders_directory(self.media_type, i)) for i in folder_scrapers]
	...
	def get_search_year(self):
		...
		if custom_year: year = custom_year
			...
			if self.active_external and get_setting('afm.search.enable.yearcheck', 'false') == 'true':
	...
	def _special_filter(self, results, key, enable_setting):
		if key == self.hevc_filter_key and enable_setting in (0,2):
			hevc_max_quality = self._get_quality_rank(get_setting('afm.filter_hevc.%s' % ('max_autoplay_quality' if self.autoplay else 'max_quality'), '4K'))
	...
	def furkPacks(self, name, file_id, download=False):
		...
		self._kill_progress_dialog()
		return afmPlayer().run(link, 'video')
