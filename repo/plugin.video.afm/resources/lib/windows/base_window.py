...
addon_skins_folder = 'special://home/addons/plugin.video.afm/resources/skins/Default/1080i/'
...
def window_manager(obj):
	def close():
		obj.close()
		clear_property('afm.window_loaded')
		clear_property('afm.window_stack')
	def monitor():
		timer = 0
		while not get_property('afm.window_loaded') == 'true' and timer <= 5:
			sleep(50)
			timer += 0.05
		hide_busy_dialog()
		obj.close()
	...
	def get_stack():
		try: window_stack = json.loads(get_property('afm.window_stack'))
		except: window_stack = []
		return window_stack
	def add_to_stack(params):
		window_stack.append(params)
		set_property('afm.window_stack', json.dumps(window_stack))
...
class BaseDialog(window_xml_dialog):
	...
	def highlight_var(self, force=False):
		...
		if not custom_var: return get_infolabel('Window(10000).Property(afm.main_highlight)')
		if 'Custom' in self.args[1]: var = custom_var
		elif force:
			if use_custom_skins(): var = custom_var
			else: var = 'Window(10000).Property(afm.main_highlight)'
		else: var = 'Window(10000).Property(afm.main_highlight)'
	...
	def set_home_property(self, prop, value):
		set_property('afm.%s' % prop, value)

	def get_home_property(self, prop):
		return get_property('afm.%s' % prop)
...
class FontUtils:
	...
	def match_font(self, size, bold, fallback):
		font_tag = 'afm_%s%s' % (size, '_BOLD' if bold else '')
